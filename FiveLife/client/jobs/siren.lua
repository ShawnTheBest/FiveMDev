...
CONTROLS
	...
	Change siren tone:			N				(Next Radio Station)
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsControlJustPressed(0, 86) or IsDisabledControlJustPressed(0, 86) then
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, 80) or IsDisabledControlJustReleased(0, 80) then
								if keyboardOpen == false then
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, 157) or IsDisabledControlJustReleased(0, 157) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- one
										SetLxSirenStateForVeh(veh, nstate)
										count_bcast_timer = delay_bcast_timer
									end
								elseif IsDisabledControlJustReleased(0, 158) or IsDisabledControlJustReleased(0, 158) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 2
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- two
										SetLxSirenStateForVeh(veh, nstate)
										count_bcast_timer = delay_bcast_timer
									end
								elseif IsDisabledControlJustReleased(0, 160) or IsDisabledControlJustReleased(0, 160) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 3
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- three
										SetLxSirenStateForVeh(veh, nstate)
										count_bcast_timer = delay_bcast_timer
									end
								end
